...
spend_by_city_concert <- data_train %>% 
  group_by(CityName) %>% 
  summarise(City_Spend = mean(predicted_dollars),
            City_Spend_sd = sd(predicted_dollars),
            total_spend = sum(predicted_dollars),
...
ggplot(fan_types_by_city, aes(x= Sport, y = expected_super_fans/1000, fill = Sport)) + 
  ...
  facet_wrap(~CityName)+
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "Sport", y = "Expected Thousands of Super Fans", title = "Super Fans by City and Sport")

ggplot(fan_types_by_city, aes(x= Sport, y = FINAL_SPEND, fill = Sport)) + 
  geom_col() +
  facet_wrap(~CityName)+
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "Sport", y = "Expected Revenue", title = "Super Fans by City and Sport")
```
