...
vars_to_impute <- c("client_pe_tkt_cnt", "financial_pct_score","e3_spend_pe_m_sports")

data_to_impute <- 
  allData %>% sample_n(5000) %>% select_if(function(x)  !is.character(x)) %>% select(-contains("int_") & -contains("_ind"))

mice_data <- mice::mice(data_to_impute ,vars = vars_to_impute,method = "rf")

imputed_5k <- complete(mice_data)
write.csv(imputed_5k, "imputed_5k.csv")
