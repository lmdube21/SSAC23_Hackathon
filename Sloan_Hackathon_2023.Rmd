...
ggplot(data_train, aes(x= predicted_dollars, fill = CityName)) +
  ...
  xlim(130,300) + 
  labs(fill = "City", x = "Average $ Per Event") +
...
fan_types_by_city = fan_types_by_city %>%
    mutate(league_revenue_tickets_percent = case_when(Sport == "Baseball" ~ FINAL_SPEND / (0.3*9780000000),
                               Sport == "Football" ~ FINAL_SPEND /(0.15*17035000000),
                               Sport == "Soccer" ~ FINAL_SPEND /(0.28*1490000000),
                               Sport == "Hockey" ~ FINAL_SPEND /(0.377*6100000000),
                               Sport == "Basketball" ~ FINAL_SPEND / (0.2238*9541000000)))


table_for_plot = read_csv("table.csv")
table_for_plot = table_for_plot[order(table_for_plot$`Percentage`, decreasing = TRUE), ]
table_for_plot %>%  
  gt() %>% 
  gt_theme_espn() %>% 
  tab_header(title = "Percent of Current League Ticket Sales Revenue" )%>%
  cols_align(
    align = "center",
    columns = 'Percentage'
  )
...
ggplot(concert_matches, aes(y = FINAL_SPEND_concert/1000000, x= FINAL_SPEND/1000000, shape = CityName, col = Sport, size = (FINAL_SPEND_concert+FINAL_SPEND)/1000000)) + geom_point() + labs(x = "Million $ of Sports Revenue", y = "Million $ of Concert Revenue", shape = "City", title= "Sports & Concert Revenue") + ggthemes::theme_economist() + guides(size =F, shape = F) +theme(legend.position="top")
``` 
